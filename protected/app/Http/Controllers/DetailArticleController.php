...
class DetailArticleController extends Controller
{
	...
	$article = DB::table('article')
	 ...
	 if($input){
		 ...
		 ->where('slide.id_category', '2')
		 ->where('article.id', $input)
	 ...
	 $articlenya = DB::table('article')->where('id_category', '2')->get();	 
	 $article = DB::table('article')
		 ->join('category', 'article.id_category', '=', 'category.id')
		 ->select('category.category','article.id','article.title', 'article.content', 'article.created_at', 'article.n_status')
		 ->where('category.id', '2')
	 ...
	 return view('admin.listartikel', compact('input','articlenya','article','slide','pages'));		 
	 }
	 ...
	 $pages="detail";
	  return view('admin.addartikel', compact('pages'));
	 }
	 ...
	 #Upload image
	  ...
	  if ($validator->fails()) {
		...
		if (Input::file('image')->isValid()) {	
		  ...
		  $x='big_'.$fileName;			  
		  $y='small_'.$fileName;
		//Resize image @upload/slide/ 		  
		  $image2 = Image::make(sprintf('upload/banner/%s', $fileName))->resize(535, 458)->save('upload/banner/'.$x);
		  $image3 = Image::make(sprintf('upload/banner/%s', $fileName))->resize(378, 274)->save('upload/banner/'.$y);
		  ...
		  return Redirect::to('cms/artikel');
		}
	  }
	  ...
	  return Redirect::to('cms/detail-article')->withPesan('Artikel baru berhasil ditambahkan.');
	 }
	 ...
	 **/ 
	 public function edit($id)
	 {
		...
		return view('admin.editartikel', compact('article','pages'));  
	 }
		 ...
		 #Upload image
		  ...
		  if ($validator->fails()) {
			...
			if (Input::file('image')->isValid()) {	
			  ...
			  echo "failed";die;
			  Session::flash('error', 'uploaded file is not valid');
			  return Redirect::to('cms/list-artikel');
			}
		  }
		}		
		$input['event_date']=date('Y-m-d',strtotime($input['event_date']));
